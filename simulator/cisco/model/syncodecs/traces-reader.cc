...
void FrameDataIterator::findNonEmpty() {
    while (m_line.size() == 0 && *m_input) {
        ...
        m_line.erase(std::find_if(m_line.begin(), m_line.end(), isChar<'%'>), m_line.end());
        //remove leading
        auto not_space = [](char c) { return !std::isspace(static_cast<int>(c)); };
        m_line.erase(m_line.begin(),
                    std::find_if(m_line.begin(), m_line.end(), not_space));
        //remove trailing
        m_line.erase(std::find_if(m_line.rbegin(), m_line.rend(), not_space).base(),
                    m_line.end());
    }
}

...
// Acknowledgments.
func (f *FeedbackAdapter) OnTransportCCFeedback(feedback *rtcp.TransportLayerCC) ([]Acknowledgment, error) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
// to Acknowledgments.
func (f *FeedbackAdapter) OnRFC8888Feedback(feedback *rtcp.CCFeedbackReport) []Acknowledgment {
	f.lock.Lock()
	defer f.lock.Unlock()

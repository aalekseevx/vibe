...
func (d *PacketDumper) logRTPPacket(info *interceptor.StreamInfo, header *rtp.Header, payload []byte, attributes interceptor.Attributes) {
	select {
...
func (d *PacketDumper) writeDumpedRTP(dump *rtpDump) error {
	...
	if d.rtpFormatBinary != nil {
		dumped, err := d.rtpFormatBinary(dump.info, dump.packet, dump.attributes)
		if err != nil {
